...
- **⚠️ 语音回复的「edge 神经语音」档会出网（2026-09-15 加第三档音源时确认）**：`voice_reply.backend` 默认 `edge`，它把**要念的那一句话**发到微软的在线语音服务（只发这一句、不上传音频、不回传数据），换来的是接近真人的音质；`sapi` 档才真正「全程本机、内容不出网」，`http` 档发到你自己的本地服务。⇒ **控制台不许再写"合成全程本机、内容不出网"这种一刀切的话**（判据 `scripts/edge_tts_selftest.py` G 段盯着这条）；用户若要"绝不外传"，把 `backend` 改回 `sapi` 即可。**试听按钮（`/api/tts/test`）必须走 `voice_models.make()`**，不许直连 `tts.make()`——否则面板写着 edge、放出来的是系统机械音。
