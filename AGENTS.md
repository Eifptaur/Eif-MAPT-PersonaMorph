...
- ✅ **已接进生产默认路径（2026-09-13，提交随本次）**：`send_text()` 改为**投递优先** —— 会话头三态闸判 `ok` 就走 `send_text_posted()`（不动光标/不抢前台/不要求可见）；判 `mismatch`/`no_ref`/抓不到就**退回真实路径**（真实路径本来就会按名字打开会话，顺手把这个尺寸的会话头**学到手**，于是下次就能走投递）。实测 `_scratch/header_diag.py`：清掉一条坏参照后**同会话连续 5 次相似度 1.000** → `check=ok` → `send_text` 返回「投递发送成功…（投递档 L5）」、**光标与前台全程未动**、DB 回读命中（local_id=561）。
