...
③**不打扰用户**：用户正在别的程序里打字时，不许把前台**长期/无故**切走。⚠️ 实测口径（2026-09-16 跨机 r15/r16）：投递链的**伪激活**会让微信**短暂真占前台**——发文字约 1.8s（r16 优化后 0.6s）、切会话约 2.9~3.2s，**随后由 `_restore_fg_until` 自动还回**。⇒ 对外一律写「不动光标、不要求可见；可能短暂置前约 1~3 秒后自动还回」，**不许写那种过头话**（判据：`scripts/background_selftest.py` F 段）。
...
|---|---|---|---|---|
| **L5 后台消息注入** | `SendMessageW`/`PostMessageW` 发 `WM_LBUTTONDOWN`/`WM_MOUSEMOVE`/`WM_LBUTTONUP`、`WM_KEY*`、`WM_CHAR`、`WM_MOUSEWHEEL`；**发前先发 `WM_ACTIVATE/WA_ACTIVE` 伪激活**（让目标自认为被激活，但**会不会短暂置前取决于微信怎么处理它**（2026-09-16 跨机实测：会短暂真占前台 0.6~3 秒后自动还回））；lParam 必须用**目标窗口客户区坐标** | ✅ 完全不动光标、不动窗口、可在后台 | **MAA 上验证有效的一档**；对微信**尚未验证**（见下方"必须先做的最小实验"） | MAA 实测：`prepare_mouse_position()` 是空实现 ⇒ 真不动光标。⚠️ 对 Chromium/Electron 类窗口的输入消息常被忽略，微信 4.x 就是这类 ⇒ **只当"待验证"用，不许先假定它能点** |
...
- **已确认（2026-09-13 15:16，决定性 · 微信 4.1.15.8）**：**L5 后台注入可行** —— 把消息**投给主窗 `Qt51514QWindowIcon`**（伪激活 `WM_ACTIVATE` → 逐字 `WM_CHAR` → `Enter`），在**微信不在前台**（前台是 Chrome）、**光标全程不动**、**前台不被抢**的条件下 **3/3 成功**（判据＝DB 回读唯一 token，2/3 干净、一次带上了探针残留字符）⇒ **最高目标的「不抢鼠标 + 不打扰用户（伪激活会短暂置前，见第③条口径）」这一环有实测支撑了。**
...
**⛔ 手写投递必须带伪激活（2026-09-15 实测，代价＝四轮实验全白做）**：任何自己写 `PostMessage` 发鼠标消息的脚本，**必须先发 `WM_ACTIVATE(1)` + `WM_NCACTIVATE(1)`**（各一发 + 约 80ms 等待），否则**连已知可用的「点笑脸弹表情面板」都没反应**，会被误判成「投递通路在这台机器上失效」——本轮就是这么错的。正解＝一律走产品 API：`agent/input_backend.select_backend(gui=gui).click(hwnd, 屏幕坐标, hover_ms, press_ms)`（内部自带 `_wake()` 伪激活 + `to_client` 屏幕→客户区换算 + 慢节奏）。配套两条：①**排查任何新控件前先把阳性对照做掉**（用一条已知可用的路径证明通路），否则阴性结论全是假阴性；②坐标一律**运行时现算**——实测同一窗口两次进入渲染区高度就不同（1139×945 vs 1139×890），写死 y 会越界。
...
- ⚠️ **面板弹层会自己抓激活**：投递点笑脸后 `GetForegroundWindow` 由主窗变成面板窗（同一微信进程内）。**"用户正在别的程序里时会不会被抢前台"尚未测**——这是最高目标第③条的待验项（发表情很可能把面板带到前台，属于「短暂置前」那一类代价）。
...
- ⇒ 之前"不带焦点时回车不提交（DB 0/3）"的缺口被**点发送按钮**绕过：**回车要焦点，点按钮不要**。⇒ 发送环（打字+提交）已可在**不碰鼠标、不预设焦点、不要求可见**下闭环（伪激活可能短暂置前，见第③条）。
...
- ✅ **已接进生产默认路径（2026-09-13，提交随本次）**：`send_text()` 改为**投递优先** —— 会话头三态闸判 `ok` 就走 `send_text_posted()`（不动光标、不要求可见；伪激活可能短暂置前约 1~3 秒后自动还回）；判 `mismatch`/`no_ref`/抓不到就**退回真实路径**（真实路径本来就会按名字打开会话，顺手把这个尺寸的会话头**学到手**，于是下次就能走投递）。实测 `_scratch/header_diag.py`：清掉一条坏参照后**同会话连续 5 次相似度 1.000** → `check=ok` → `send_text` 返回「投递发送成功…（投递档 L5）」、**光标与前台全程未动**、DB 回读命中（local_id=561）。
- ⚠️ **坏参照会长期误判**（实测：一条早期自动学到的坏参照让同一会话长期判 0.653 < 0.90 ⇒ 一直退回真实路径）⇒ 两处收口：①`_learn_chat_header()` 现在**先探面板左沿，探不到就不学**（口径可疑的图不许进参照库）；②真要修就**删 `data/chat_headers.json` 重新 seed**（`chat_header.seed_from_main()`），立刻恢复 1.000。
...
- **产品级口径：绝大部分不出网，出网一律是「可选项」（用户 2026-09-15 定，原话：「关于"全程不出网"的条款，你可以写成"**绝大部分不出网**"，就是**出不出网是可选项**」）**：⇒ 落地三条：①**默认路径尽量不出网**（读库/识别/抽帧/记忆/图库都是本机），②**每一处会出网的能力都必须配一条"不出网的替代档"**，并把档位摆在用户面前（例：语音回复三档＝`sapi` 全程本机 / `edge` 走微软在线语音服务 / `http` 发到你自己的本地服务；生图＝本地 ComfyUI / 在线 API；模型＝本机模型 / 云 API），③**面板上按"当前选的那一档"如实写清楚**，不许一刀切地说"全程不出网"（判据 `scripts/edge_tts_selftest.py` G 段）。
  ⚠️ 具体到语音：`voice_reply.backend` 默认 `edge`，它把**要念的那一句话**发到微软的在线语音服务（只发这一句、不上传音频、不回传数据），换来接近真人的音质；**要"绝不外传"就把 `backend` 改回 `sapi`**。另：**试听按钮（`/api/tts/test`）必须走 `voice_models.make()`**，不许直连 `tts.make()`——否则面板写着 edge、放出来的是系统机械音。
