...
- **已确认**：L5 `PostMessage`（投给渲染子窗：伪激活 `WM_ACTIVATE` → `WM_CHAR` → `Enter`）在**微信处于前台且输入框已聚焦**时**确实能把消息发出去**（1 次阳性，判据＝DB 回读 `w0cal276w0a20182903`）⇒ 这条路不是想当然，是真通。
- **未确认（不许提前下结论）**：微信**非前台**时能不能发 —— 本轮 9 次全 0，但那 9 次注入时输入框本来就没焦点（聚焦点击被前面的窗口吃掉）⇒ **这 9 次不能当证据**。
...
- 本机几何（1160×900 窗口）：输入框外框 y≈724..969、文字行 y≈754..809；落点 `(795,785)`；文字区探针 `(460,752,1150,812)`。**点消息区会把输入框焦点抢走**（打什么进不去）——这是本轮最容易踩的假阴性来源。
