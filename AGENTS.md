...
- ⚠️ **兼容性实测（2026-09-13）暴露两个必须修的口径**：①**指纹锚点**用了 `0.26×窗口宽` 假设会话列表按比例缩放——实测它是**固定像素宽（约 300px）**，窗口一变窄文字带就漂进会话列表（相似度掉到 0.70~0.90）⇒ 要先探测面板左沿再放带；②**指纹靠 `ImageGrab` 抓屏** ⇒ 窗口被遮挡/隐藏/最小化时抓到的不是它（实测预检 1.000、发送瞬间 0.755）⇒ 应改成 `PrintWindow(PW_RENDERFULLCONTENT)` 抓窗口自身画面。**这两条修完，"任何尺寸/被遮挡"才算过。**
