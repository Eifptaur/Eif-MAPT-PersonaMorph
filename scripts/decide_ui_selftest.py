...
ok("选项 key 没有写死在页面里（按钮靠后端 options 数据驱动）", "upgrade_adapter" not in H)
ok("四个选项名只出现在给用户看的提示里，没有第二份写死的选项表",
   H.count("一键升级适配层") == 1 and H.count("微信本身要处理") == 1,
   "一键升级适配层×%d / 微信本身要处理×%d" % (H.count("一键升级适配层"), H.count("微信本身要处理")))
...
ok("升级命令回显给人（可复制那条命令）", "act.cmd" in H and "confirmBox(" in H)
