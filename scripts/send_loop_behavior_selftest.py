...
#   "已经记账的自我文本"，而这一枪**没记账**。修法两条：①超时窗口放宽到 ≥6s（这段等待发生在
#   **还前台之后**，不占前台）；②回退老链**之前**先复核"这一枪是不是其实已经发出去了"。
ok("I2a 快路径的超时窗口已放宽（2.6s ⇒ ≥6s；这段等待在还前台之后，不占前台）",
   "deadline = time.time() + 6.5" in _src_w,
   [l.strip() for l in _src_w.splitlines() if "deadline = time.time() +" in l][:1])
ok("I2b 回退老链之前先做「是不是已经发出去了」的复核（复核到就**不再重复打字**）",
   "快路径超时后的复核" in _src_w and "不再重复打字" in _src_w)
ok("I2c 复核是**数据判据**（按 local_id 比开工前更新 + 文本 + 时间窗），不是看返回值",
   "local_id" in _src_w.split("快路径超时后的复核")[0][-1600:]
   and "90000" in _src_w)
