...
_FALLBACK_SELF_REF = ("我不打算", "不打算回", "不回复", "不打算说", "没什么可说", "没什么好说",
                      ...
                      "会话没对上", "没能对上", "保持安静", "安静一点", "本轮", "这轮不", "这轮没",
                      "不需要回复", "不需要回", "不必回复", "无需回复", "不参与", "就不掺和")

# 兜底文本的"外壳"（内部结论常被括号/方括号括起来，如 `(本轮无法发出发言，保持安静。)`）
# ⇒ 比对前先剥掉首尾这些符号，否则首尾带括号会削弱匹配（也方便按"整条都是括号"再判一次）。
_FALLBACK_WRAP = "（）()【】[]《》〈〉“”\"'‘’ \t\r\n"
...
def _fallback_send_ok(text: str, cfg=None):
    ...
    try:
        ...
        if s.get("fallback_block_selfref", True) is not False:
            core = str(text or "").strip().strip(_FALLBACK_WRAP)
            for k in _FALLBACK_SELF_REF:
                if k in text or (core and k in core):
