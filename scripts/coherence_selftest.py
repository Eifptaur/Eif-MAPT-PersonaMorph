...
try:
    ...
    _bad_txts = ["(本轮无法发出发言，保持安静。)", "本轮无法发出发言", "发送失败，稍后再试",
                 "图没能发出", "没发出去", "会话没对上", "保持安静", "（这轮不回）"]
    _leak = [t for t in _bad_txts if _fn(t, _c)[0]]
    ck("E13 内部失败结论/保持安静 一并拦下（含首尾括号包装）", not _leak, str(_leak))
    ck("E13a 正常群发言不被误伤（不因为加了失败词就把好话也拦了）",
       _fn("这图真好看", _c)[0] and _fn("我先去吃饭了", _c)[0] and _fn("来了 别催了", _c)[0])
