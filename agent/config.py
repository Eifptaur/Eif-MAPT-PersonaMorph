...
DEFAULT_CONFIG = {
    ...
    "wechat": {
        ...
        "group_name_white_list": [],        # 监听目标群：**群 wxid**（老配置写群名也认；空 = 所有群）
    ...
    "store": {
        ...
        # ⛔ 2026-09-22 加（B站网友要的「提前设定信息」）：**预设信息**——按会话设定背景事实、
        #   可设截止日期、到期自动舍弃、**只在本会话相关时注入**、结构上不外泄到别的会话。
        #   实现见 `agent/briefs.py`；控制台「预设信息」面板一行行编辑；存 `data/briefs.json`。
        "briefs": {
            "enabled": True,          # 关掉＝一条都不注入（已有条目原样留着，不删）
            "inject_max": 5,          # 单轮最多注入几条（相关才注入，宁少不多）
        },
    },
    # ── 上云（预留接口：默认关＝不上传任何数据）──────────────────────────────
