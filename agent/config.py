...
DEFAULT_CONFIG = {
    ...
    #   外加一次**位置标定**（微信那个"进录音态"的圆圈位置随版本/尺寸变，存下面两个比例里）。
    "voice_strip": {
        "enabled": False,        # 总开关（默认关：没装虚拟声卡的用户不该看到它）
        "record_btn": [],        # 进录音态的圆圈（渲染区比例 [x, y]）；空＝还没标定，拒发并提示标定
        "send_btn": [0.932, 0.945],   # 录音态里那个绿簇"发送"（渲染区比例）
    ...
    "voice_reply": {
        ...
        "fallback_file": True,   # form=strip 但前提不齐（没虚拟声卡/没标定）时：如实回退成音频文件并说明；false＝干脆不发
