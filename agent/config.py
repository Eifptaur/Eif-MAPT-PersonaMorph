...
DEFAULT_CONFIG = {
    ...
    "wechat": {
        ...
        # 只走后台（**2026-09-16 默认改为 True** —— 用户报「为什么鼠标会滑我的控制台」：
    ...
    # ── 上云（预留接口：默认关＝不上传任何数据）──────────────────────────────
    # 用户口径（2026-09-14）：「上云三件先不做，但是留可以输网址的接口，在 UI 里面也要相应地做一切的交互设计」
    "cloud": {
    ...
    # 红线（用户口径"只允许被动或显式开启"）：定时消息只能设到**当前会话**、到点仍过风险闸门、
    ...
    "feedback": {
        ...
        # 可选：Web3Forms（或同类"公开可提交"服务）的 access key —— 2026-09-16 加。
        # 为什么要有它（用户原话：「应该只有一个小小的窗，填什么类型、具体内容，然后点发送就行了呀，
        # 用户为什么还要在意那么多」）：自建中转与自己邮箱+授权码**两条路都要用户自己配**，而收件凭据
