...
<div class="shell">
  ...
  <main class="main">
    ...
    <section id="sec-tts" class="card" data-sec>
      ...
      <div class="row"><label>本地服务地址</label><div class="grow"><input type="text" data-cfg="voice_reply.http_url" placeholder="如 http://127.0.0.1:9880/tts">
        ...
        <span class="hint"><b>这是你自己电脑上跑起来的服务，不是网页、也不是我们帮你托管</b>：像 <b>GPT-SoVITS</b>（文字→音频，可克隆音色）这类本地模型，用它自带的 <code>api.py</code> 起一个本机服务（默认 <code>http://127.0.0.1:9880/tts</code>），模型文件放在它自己的目录里加载；这里填它的地址即可。<b>不填＝用上面「本机系统声音」那一档</b>（系统自带、零配置）。回音频字节（audio/wav）直接用；回配置格式就在下面填字段名。</span></div></div>
      ...
      <div class="row"><label>变声服务</label><div class="grow"><input type="text" data-cfg="voice_reply.vc_url" placeholder="如 http://127.0.0.1:7897/infer（留空＝不变声）">
        ...
        <span class="hint">这也是**你自己电脑上跑的服务**（RVC 自带的 <code>api.py</code>，默认 <code>http://127.0.0.1:7897/infer</code>），不是网页版。这是**第二段**：先合成出人声，再送去<b>变声</b>成你要的音色。<b>GPT-SoVITS 是「文字→音频」</b>（填上面那个地址就够）；<b>RVC 是「音频→音频」</b>（要 .pth 音色模型 + .index 检索索引），所以要在这里再填一个地址——两段串起来才是 RVC 的效果。</span></div></div>
      ...
      <div class="row"><label>念法纠正</label><div class="grow"><textarea data-cfg="voice_reply.pronounce" rows="3" spellcheck="false" placeholder="行行行=行，行，行&#10;银行=银 行"></textarea>
        <div class="hint">一行一条「原文=念法」，送进引擎**之前**替换。连续同字（行行行/哈哈哈）与没标点的长句我们**已经会自动断句**（所以不会再念成"行行hang行"），这里只放它猜不对的例外。</div></div></div>
...
const WHALE_CURSOR = (function(){
  ...
  function set(on){
    enabled = !!on;
    if(on){ document.documentElement.classList.add('whale-cursor'); preload(); apply(); }
    else {
    ...
    document.addEventListener('keydown', function(e){
      ...
      try{
        if(window.chrome && window.chrome.webview && window.chrome.webview.postMessage){
          window.chrome.webview.postMessage('pm-esc-exit-fullscreen');
        }
      }catch(_e){}
    }, true);
  }catch(e){}
        ...
        if(g1 && !md.error){
          ...
          g1.textContent = ig.enabled
            ? (n ? ('已开 · ' + n + ' 个后端可用（' + (ig.backends || []).map(function(b){ return b.id; }).join(' / ') + '）')
                 : '已开：本机没探到生图服务、也没手填 ⇒ 调 gen_image 会如实回「还没配后端」')
            : '未开启（默认关）';
...
const GUIDES = {
  ...
  tts: {
    ...
    steps: [
      ...
      '**念错字 / 不断句**：连续同一个字（行行行、哈哈哈）会自动断成「行，行，行」，没标点的长句按呼吸断句；引擎仍念不对的，写进下面「念法纠正」，一行一条 `原文=念法`（例如 `银行=银 行`）。',
      '**音源与出网**：系统声音档全程本机、不出网（机械音）——**想全程不出网就选系统声音那一档**；edge 档会把**要念的这句话**发到微软在线语音服务（只为换音质，不发音频、不回传数据）；http 档发到你自己填的本地服务。',
      '「试听一句」只合成、只播出来给你听，**不会发到任何会话**。'
    ],
    copy: [{label: '念法表写法', text: '行行行=行，行，行\n银行=银 行'}],
  ...
  imggen: {
    ...
    intro: '这条链是**本机跑**的：解析要求 → 挑一个生图后端 → 生成 → **过过滤链** → 才发。后端**程序会自己找**（探本机常用的 A1111 / ComfyUI / Fooocus / InvokeAI，只读探活；在线免密钥的 pollinations 要开了「允许出网」才用），一个都没探到、也没手填时才会如实回「还没配后端」。',
    steps: [
