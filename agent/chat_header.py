...
def detect_band_y0(img, x0: int, x1: int, default_y0: int = None, max_scan: int = 96,
    ...
    "投递切会话 + 内容级复核"兜底，兜不住就整条拒发 —— 正是网友说的「读得到、只是发不出去」
    「艾特它它不回复」。归因文案当时还把它写成"窗口最小化/抓不到画面"，方向完全不对。
    ...
    try:
        ...
        step = max(1, (x1 - x0) // 60)
        run, run_end = 0, None
        for y in range(0, min(int(max_scan), h)):
            dark = tot = 0
            for x in range(x0, x1, step):
                tot += 1
                if px[x, y] < DARK:
                    dark += 1
            if tot and (dark / float(tot)) >= BAND_FULL_DARK:
                run += 1
                run_end = y
                continue
            if run >= 4:                      # 那条横条结束了
...
def remember(chat_id: str, fp, note: str = "", path: str = None, size: str = "*") -> dict:
    ...
    data = load(path)
    if is_blank(fp):
        log.warning("拒绝记住空白会话头指纹：%s（尺寸 %s）—— 窗口最小化/抓不到画面，"
                    "或**文字带压在自绘标题条上**（见 `detect_band_y0`）", chat_id, size)
        return data
    ...
    if len(_ink) < 4:
        log.warning("拒绝记住**墨列太少**的会话头指纹：%s（尺寸 %s）—— 只有 %d 个有墨的列"
                    ...
                    "mismatch（＝漏发）", chat_id, size, len(_ink),
                    ",".join(str(i) for i in _ink[:8]))
        return data
...
def reference(chat_id: str, path: str = None, size: str = None, strict: bool = False) -> list:
    ...
    """
    def _fresh(v) -> bool:
        return bool((v or {}).get("fp")) and int((v or {}).get("band") or 0) == BAND_VERSION

    ent = (load(path).get(str(chat_id)) or {})
    sizes = ent.get("sizes") or {}
    if not sizes and ent.get("fp"):
        return [] if strict else list(ent.get("fp") or [])
    if size and _fresh(sizes.get(str(size))):
...
def is_blank(fp) -> bool:
    ...
    ⚠️ 2026-09-21（第八轮审计 **V-R8-3**）：**查过、决定保留判据二**，别再删它。两条理由：
      ...
      · 但仍保留：①它是 `is_blank()` 的**返回值**，而 `is_blank()` 在别处**单独使用**
        （`scripts/collect_report.py` 的体检报告那里根本不看 `degenerate_reason`）⇒ 删掉会静默
        改掉那边的口径；②这是**用户真报障换来的守卫**（「机器人有时不回话」），而且"冗余"依赖
        「`degenerate_reason` 判据③ 今后不变」这个耦合 —— 省 2 行不划算，破守备代价大。
      · ⇒ 要收「标题带里只有两条竖线、没有字」那类帧，请**在入库入口**收
        （`remember()` 的「有墨的列 < 4 ⇒ 只记日志不入库」那道闸），**不要在这里抬阈值**。
    """
    try:
...
def grab_render(gui=None, render=None, tries: int = 12):
    ...
    try:
        ...
        if not main_pid:
            log.warning("拿不到主窗 PID（main_hwnd 缺失）⇒ **不退回抓屏**：抓屏可能读到别人家的像素")
            return None
        if _region_occluded(render, main_pid):
...
def check(chat_id: str, gui=None, path: str = None,
    ...
    同一会话在 1000×760 / 1400×1000 下相似度掉到 0.65）。所以设计成：
      · 当前尺寸**有**参照且不匹配 ⇒ `mismatch`（**拦投递档**，这是"用户在同一个布局里切了会话"的常见风险。
        ...
        不抢鼠标，**只有那时才真的不发**）。⇒「能发的消息必须发得出去」靠的是**那条兜底链**，
        不是靠放宽这里）；
