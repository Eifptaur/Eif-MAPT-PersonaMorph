...
def _builtin_tool_defs() -> list:
    return [
        ...
        },
        {
            ...
            "description": "把一句话合成成语音发到当前会话（本机 TTS，零下载）。**默认发「真语音条」（微信语音气泡，对方点开就听）**；没有虚拟声卡/合成引擎时按设置**如实回退成音频文件**，并在返回里说明「为什么这次是文件」——照实说，别把文件说成语音条。适合「用语音回一句」。功能没开 / 文本太长 / 没合成引擎 / 发失败时它会返回原因，照原因如实说明即可。",
            "parameters": {
                "type": "object",
        ...
        if result["failed"]:
            ...
            for _idx, _f in enumerate(_why):
                ...
                try:
                    ...
                    #   从未生效）；而且原来是"拿失败回执的前 20 字去 messages 里盲找"，同前缀的两条会
                    #   入队**已发成功**那条 ⇒ 补发＝重复发。
                    #   ⇒ 改成用**发送层回传的原文**（`sender.failed[*]["src"]`，已按 parts 对齐）。
                    if _sr.retryable(_f["error"]):
                        _rawf = result["failed"][_idx] if _idx < len(result["failed"]) else {}
                        _full = str(_rawf.get("src") or "")
                        if _full.strip():
                            _r = _sr.enqueue(ctx["chat_key"], _full, _f["error"])
                            if _r.get("ok"):
                                _queued += 1
                except Exception as _e:
                    log.warning("重试入队失败（不影响本次返回）：%s", _e)
...
def _exec_forward_media(ctx, args):
    ...
    try:
        ...
        if not bool((cfg.get("send") or {}).get("file_forward_optin")):
            return _ok("转发视频/文件默认关闭：它必须过一次系统「选择文件」对话框，那一下会让微信**短暂占前台约 0.5~3 秒**（与「不动鼠标、不长时间占前台」的最高目标有冲突），"
